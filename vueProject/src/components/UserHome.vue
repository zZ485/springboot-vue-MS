...
export default {
  ...
  created () {
    ...
    this.axios({
      method: 'post',
      url: 'http://127.0.0.1:8080/ssmProject_war_exploded/user/selectUserPage',
      data: postData1
    }).then(response => {
    ...
    this.axios({
      method: 'post',
      url: 'http://127.0.0.1:8080/ssmProject_war_exploded/user/getRowCount',
      data: postData
    }).then(response => {
  ...
  methods: {
     ...
     * 分页
     */
    handlePageChange () { // console.log(`当前页: ${this.currentPage}`);
      let postData = this.qs.stringify({
        page: this.currentPage,
        userName: this.search1,
        userSex: this.search2
    ...
    addUser () {
      ...
      this.axios({
        method: 'post',
        url: 'http://127.0.0.1:8080/ssmProject_war_exploded/user/createUser',
        data: postData
      }).then(response => {
        ...
        this.emptyUserData(); // console.log(response);
      }).catch(error => {
        console.log(error);
      });
    },
    ...
    handleDelete (index, row) { // console.log(index, row);
      this.$confirm('删除操作, 是否继续?', '提示', {
        ...
        this.axios({
          method: 'post',
          url: 'http://127.0.0.1:8080/ssmProject_war_exploded/user/deleteUserById',
          data: postData
        }).then(response => {
          this.getRowCount();
    ...
    handleDeleteList () {
      ...
      console.log(userIds); //  let userIds= this.multipleSelection.map(item => item.userId).join()
      this.$confirm('删除操作, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
        ...
        this.axios({
          method: 'post',
          url: 'http://127.0.0.1:8080/ssmProject_war_exploded/user/deleteUserByIdList',
          data: postData
        }).then(response => {
          this.getRowCount();
          if (this.total % 5 === 1 && this.currentPage >= 1) {
    ...
    updateUser () {
      ...
      this.axios({
        method: 'post',
        url: 'http://127.0.0.1:8080/ssmProject_war_exploded/user/updateUserById',
        data: postData
      }).then(response => {
        this.handlePageChange();
        this.getRowCount();
